...
export async function generatePdfStructure({
  fopCustomer,
  dto,
  totalAmount,
  rateUahToEur,
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      if (bankDetails?.iban)
        lines.push(`П/р в форматі IBAN: ${bankDetails.iban}`);
      ...
      if (bankDetails?.edrpou)
        lines.push(`код по ЄДРПОУ ${bankDetails.edrpou}`);
      ...
      doc.end();
      console.log('jkbkjbkjkj', filePath)
      return new Promise((resolve, reject) => {
        stream.on('finish', () => resolve(filePath));
        stream.on('error', (h) => console.log(h, '666'));
      });
    } catch (err) {
      reject(new Error(`Unexpected PDF error: ${(err as Error).message}`));
    }
  });
}
